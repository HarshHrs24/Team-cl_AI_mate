...
def timeline_prepare(df,model):
    if model=="Heat wave":
        df['occurence of heat wave']= df["yhat_upper"].apply(lambda x: "yes" if x >= 44.9 else "no")
        df=df.iloc[4017:]
...
with st.container():
    ...
    with c1:
        ...
        # st.write(forecast)
        yhat=float(forecast.loc[forecast['ds']==d, 'yhat']).format("%2f")
        yhat_upper=float(forecast.loc[forecast['ds']==d, 'yhat_upper'])
        yhat_lower=float(forecast.loc[forecast['ds']==d, 'yhat_lower'])
